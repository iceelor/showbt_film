//初始化
function C_slider(frame, list, Lframe, Llist, forwardEle, backEle, scrollType,
		LscrollType, acitonType, autoInterval) {
	this.frame = frame;
	this.list = list;
	this.Lframe = Lframe;
	this.Llist = Llist;
	...
	// 定论鼠标划过时，自动轮换的处理
	$(
			"#" + this.frame + ",#" + this.Lframe + ",#" + this.forwardEle
					+ ",#" + this.backEle).bind('mouseover', function() {
		clearTimeout(self.autoExt);
	});

	$(
			"#" + this.frame + ",#" + this.Lframe + ",#" + this.forwardEle
					+ ",#" + this.backEle).bind('mouseout', function() {
		clearTimeout(self.autoExt);
...
C_slider.prototype.go = function(index) {
	...
	$("#" + this.Llist + " > li").removeClass("item-selected");
	$("#" + this.Llist + " > li").eq(index).addClass("item-selected");
	$("#" + this.list + " > li").find(".focus-area-thumb-type").css("display","none");
	$("#" + this.list + " > li").find(".focus-area-thumb-bottom-bd").css("display","none");
	$("#" + this.list + " > li").find(".focus-hidden").css("display","none");
	$("#" + this.list + " > li").eq(index).find(".focus-area-thumb-type").css("display","");
	$("#" + this.list + " > li").eq(index).find(".focus-area-thumb-bottom-bd").css("display","");
	$("#" + this.list + " > li").eq(index).find(".focus-hidden").css("display","");
	// 对于缩略图的滚动处理
	if (this.LscrollType == "left") {
		if (this.slideLength * this.lListWidth > this.lFrameWidth) {
			...
			if (hiddenSpace > 0) {
				if (hiddenSpace + this.lFrameWidth <= this.slideLength
						* this.lListWidth) {
					$("#" + this.Llist).animate({
					...
					var endHidden = this.slideLength * this.lListWidth
							- this.lFrameWidth;
					$("#" + this.Llist).animate({
		...
		if (this.slideLength * this.lListHeight > this.lFrameHeight) {
			...
			var hiddenSpace = this.lListHeight * this.currentSlide
					- spaceHeight;

			if (hiddenSpace > 0) {
				if (hiddenSpace + this.lFrameHeight <= this.slideLength
						* this.lListHeight) {
					$("#" + this.Llist).animate({
					...
					var endHidden = this.slideLength * this.lListHeight
							- this.lFrameHeight;
					$("#" + this.Llist).animate({
